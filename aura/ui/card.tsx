...
function Card({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      data-slot="card"
      className={cn(
        "bg-card text-card-foreground flex flex-col gap-4 rounded-2xl border border-border/60 py-5 px-6 shadow-[var(--aura-shadow-card)] transition-all duration-300 ease-out hover:shadow-[var(--aura-shadow-card-hover)]",
        className
      )}
      {...props}
    />
