...
function Card({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      data-slot="card"
      className={cn(
        "orbit-hover-offset bg-card text-card-foreground flex flex-col gap-5 rounded-lg border border-foreground py-5 transition-colors",
        className
      )}
      {...props}
    />
...
function CardTitle({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn("leading-none font-bold font-heading text-lg", className)}
      {...props}
    />
  )
}
